...
export default async function B2bDashboardPage() {
  ...
  // Exact P&L & Accounting Metrics from the user's favorite design
  const pnlData = {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">Estado de Resultados & Márgenes Financieros</h1>
            <p className="text-slate-500 text-sm mt-1">Visión consolidada de ventas, costos de operación, ganancia neta e impuestos (SII + Banco).</p>
          </div>
        ...
        {/* User's Favorite Top KPI Cards Row */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">MARGEN BRUTO</span>
                <PieChart className="w-5 h-5 text-emerald-600" />
              </div>
              <h3 className="text-3xl font-black text-emerald-600 mt-2">{pnlData.grossMarginPercent}%</h3>
          ...
          <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">MARGEN NETO (EBITDA)</span>
                <TrendingUp className="w-5 h-5 text-blue-600" />
              </div>
              <h3 className="text-3xl font-black text-blue-600 mt-2">{pnlData.netMarginPercent}%</h3>
          ...
          <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">PUNTO DE EQUILIBRIO</span>
                <BarChart3 className="w-5 h-5 text-purple-600" />
              </div>
              <h3 className="text-3xl font-black text-slate-900 mt-2">${pnlData.breakEvenPoint.toLocaleString("es-CL")}</h3>
          ...
          <Card className="border border-slate-200 shadow-sm bg-white rounded-2xl">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">IVA ESTIMADO A PAGAR</span>
                <Receipt className="w-5 h-5 text-amber-600" />
              </div>
              <h3 className="text-3xl font-black text-amber-600 mt-2">${pnlData.estimatedIvaPayment.toLocaleString("es-CL")}</h3>
              <p className="text-xs text-slate-500 mt-1">Proyección F29 del mes</p>
            </CardContent>
          </Card>
        </div>

        {/* User's Favorite Main P&L Statement Card */}
          ...
          <CardContent className="p-6 space-y-4">
            <div className="space-y-3">
              <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50">
                ...
                <span className="font-extrabold text-slate-900 text-lg">${pnlData.totalRevenue.toLocaleString("es-CL")}</span>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-red-50/50">
                ...
                <span className="font-bold text-red-600 text-lg">-${pnlData.costOfGoodsSold.toLocaleString("es-CL")}</span>
              </div>

              <div className="flex items-center justify-between p-4 rounded-xl bg-emerald-50 border border-emerald-200">
                <span className="font-bold text-emerald-900 text-sm">(=) Ganancia Bruta</span>
                <span className="font-extrabold text-emerald-700 text-xl">${pnlData.grossProfit.toLocaleString("es-CL")} ({pnlData.grossMarginPercent}%)</span>
              </div>

              <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50">
                <span className="font-semibold text-slate-700 text-sm">(-) Gastos Operativos (Arriendos, Sueldos, Marketing)</span>
                <span className="font-bold text-slate-800 text-lg">-${pnlData.operatingExpenses.toLocaleString("es-CL")}</span>
              </div>

              <div className="flex items-center justify-between p-5 rounded-2xl bg-slate-900 text-white shadow-xl">
                <div>
                  <span className="font-extrabold text-lg block text-emerald-400">(=) Utilidad Neta Final (EBITDA)</span>
                  <span className="text-xs text-slate-400">Resultado final para retiro o reinversión</span>
                </div>
                <span className="font-black text-3xl text-emerald-400">${pnlData.netProfit.toLocaleString("es-CL")} CLP</span>
              </div>
            </div>
          </CardContent>
        </Card>

        {/* Configured GPS Stores List */}
        <Card className="border border-slate-200 shadow-sm bg-white rounded-3xl">
          <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4 p-6">
            <CardTitle className="text-lg font-bold text-slate-900 flex items-center gap-2">
